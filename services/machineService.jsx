...
const machineService = {
  ...
  checkAndAutoUnbookExpiredMachines: async () => {
    try {
      console.log('Checking for expired machine bookings in background');
      
      // Get all machines that are currently in use
      const inUseMachinesSnapshot = await firestore()
        .collection('machines')
        .where('status', '==', 'in-use')
        .get();
      
      if (inUseMachinesSnapshot.empty) {
        console.log('No machines currently in use');
        return;
      }
      ...
      // Check each machine
      inUseMachinesSnapshot.forEach(doc => {
        const machineData = doc.data();
        
        // Skip if already being processed (prevent race conditions)
        if (alreadyAutoUnbooked[doc.id]) {
          return;
        }
        
        // Check if machine booking has expired
        if (machineData.expiryTime && new Date(machineData.expiryTime) < now) {
          console.log(`Machine ${doc.id} booking has expired. Auto-unbooking.`);
          
          // Mark as being processed
          alreadyAutoUnbooked[doc.id] = true;
          
          // Apply penalty to user (add to promises array)
          if (machineData.bookedBy) {
            penaltyPromises.push(userService.applyPenalty(machineData.bookedBy));
          }
          ...
          if (!updatedLastUses.length || 
              (machineData.bookedBy && updatedLastUses[0].email !== machineData.bookedBy)) {
            const newUserEntry = {
              email: machineData.bookedBy || 'Unknown',
              name: machineData.userName || 'Unknown User',
              mobile: machineData.userMobile || '',
              timestamp: new Date().toISOString(),
            };
          ...
          // Update machine status in batch
          batch.update(doc.ref, {
            inUse: false,
            bookedBy: null,
            status: 'available',
            lastUsedBy: machineData.bookedBy || null,
            lastUserName: machineData.userName || 'Unknown User',
            lastUserMobile: machineData.userMobile || '',
            lastUsedTime: new Date().toISOString(),
            autoUnbooked: true,
          ...
          // Set timeout to clear the flag after 1 minute
          setTimeout(() => {
            delete alreadyAutoUnbooked[doc.id];
          }, 60000);
        }
      });
      ...
      if (batch._mutations && batch._mutations.length > 0) {
        await batch.commit();
        console.log(`Auto-unbooked ${batch._mutations.length} expired machines`);
      }
      
      // Wait for all penalty applications to complete
      if (penaltyPromises.length > 0) {
        await Promise.all(penaltyPromises);
        console.log(`Applied penalties to ${penaltyPromises.length} users`);
      }
      
      return true;
    } catch (error) {
      console.error('Error in background check for expired machines:', error);
      return false;
    }
  }
};
